...
class PlacesService:
    ...
    def find_place_from_text(self, query: str) -> dict[str, str]:
        """Fetch place details using a text query."""
        self._check_key()
    ...
    def get_photo_urls(self, photos: list[dict[str, Any]], maxwidth: int = 400) -> list[str]:
        """Extract photo URLs from the 'photos' list."""
        photo_urls = []
        for photo in photos:
    ...
    def get_map_url(self, place_id: str) -> str:
        """Generate the Google Maps URL for a given place ID."""
...
def map_tool(key: str, tool_context: ToolContext) -> dict[str, Any]:
    """Inspect the pois stored under the specified key in the state.
    ...
    for poi in pois:  # The pydantic object types.POI
        ...
        poi["place_id"] = result.get("place_id", None)
        poi["map_url"] = result.get("map_url", None)
